    ...
    public prepareWalls(): void {
        ...
        for (let x = 0; x < this.room.getModelMaltrix().length; x++) {
            for (let y = 0; y < this.room.getModelMaltrix()[x].length; y++) {
                isCorner = MapTypeChecker.checkWallCorner(
                    x,
                    y,
                ...
                isLast = false;
                if (
                    !this.room.getModelMaltrix()[x + 1] ||
                    (this.room.getModelMaltrix()[x + 1] &&
                        this.room.getModelMaltrix()[x + 1][y] == 0)
                ) {
                    isLast = true;
                }
                ...
                if (
                    y <= minY &&
                    this.room.getModelMaltrix()[x][y] != 0 &&
                    !(x == doorX && y == doorY)
                ) {
                    if (minY > y) {
                        minY = y;
        ...
        for (let y = 0; y < this.room.getModelMaltrix()[0].length; y++) {
            for (let x = 0; x < this.room.getModelMaltrix().length; x++) {
                ...
                isLast = false;
                if (
                    !this.room.getModelMaltrix()[x][y + 1] ||
                    (this.room.getModelMaltrix()[x][y + 1] &&
                        this.room.getModelMaltrix()[x][y + 1] == 0)
                ) {
                    isLast = true;
                }
