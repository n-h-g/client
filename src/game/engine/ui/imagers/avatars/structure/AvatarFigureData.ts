...
import IFigureData, { ISet, CA, IPart, IPaletteColor } from "../gamedata/IFigureData";
...
export default class AvatarFigureData {
    ...
    private loadPalettes(palettes: { [key: string]: { [key: string]: IPaletteColor } }): void {
        if(palettes) {
    ...
    private loadSetTypes(setTypes: { [key: string]: CA }): void {
        if(setTypes) {
            for(let setType of Object.keys(setTypes)) {
                this.setTypes.set(setType, new SetType(setTypes[setType] as CA))
            }
        }
    }
