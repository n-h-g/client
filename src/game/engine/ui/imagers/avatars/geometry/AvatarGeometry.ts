...
export default class AvatarGeometry extends Geometry {
    ...
    public constructor(geometry: IGeometry) {
        ...
        this.cameraPoint = new Point3d(geometry.camera.x, geometry.camera.y, geometry.camera.z);
    ...
    private loadGeometryTypes(types: IType[]) {
        if(types && types.length > 0) {
            for(let geometryType of types) {
                ...
                let bodyParts = new Map<string, GeometryBodyPart>();
                
                if(geometryType.bodyParts && geometryType.bodyParts.length > 0) {
            ...
            if(canvas.geometries && canvas.geometries.length > 0) {
                canvas.geometries.forEach((geometry: IGeometryElement) => {
                    ...
                    if (canvas.scale == "h") {
                        ...
                        const avatarCanvas = new AvatarCanvas(canvas.scale, geometry);
