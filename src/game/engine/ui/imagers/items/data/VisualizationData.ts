...
import { IDirection, IVisualization } from "../../../../../core/ui/imagers/items/IVisualization";
...
export class VisualizationData {
    ...
    public hasAnimationForLayer(animation: number, layer: number): boolean {
        ...
        let data = this._animations.get(animation)

        if(!animation) return false;
        let layerData = data.getLayer(layer)
    ...
    public loadAnimations(animations: { [key: string]: IAnimation; }) {
        for(let animation of Object.keys(animations)) {
            this._animations.set(parseInt(animation), new AnimationData(parseInt(animation), animations[animation]))
        }
    }
    
    public loadDirections(directions:  { [key: string]: IDirection }) {
        for(let direction of Object.keys(directions)) {
            this._directions.set(parseInt(direction), new DirectionData(parseInt(direction), directions[direction]))
        }
    }
