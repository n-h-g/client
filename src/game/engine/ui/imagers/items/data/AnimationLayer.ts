import { IAnimationLayer, IAnimationLayerFrameSequence } from "../../../../../core/ui/imagers/items/IAnimationLayer";
...
export class AnimationLayer {
    ...
    public loadSequences(sequences: { [index: number]: IAnimationLayerFrameSequence; }) {
        for(let sequence of Object.keys(sequences)) {
            this._sequences.set(parseInt(sequence), new AnimationFrameSequence(sequences[sequence]))
        }
    }

    public getFrame(direction: number, frameId: number) {
        let sequence = this._sequences.get(direction)
